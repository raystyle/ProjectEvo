...
- **分层原则**：SKILL.md 只留原语概览；references/ 每篇一主题自包含完整参考；实现代码唯一来源是插件内 `skills/project-evo/scripts/`（PEP 723 零依赖；禁字规则唯一权威 `mdrules.py`，check PE-12/scan/md-guard 三面同源）
...
> skill 源码唯一位置是 `plugins/project-evo/skills/project-evo/`；分发走插件市场（根双清单 + 插件双 manifest），无内嵌副本（双漂移守卫已随单源化取消，清单一致性守卫接棒）。
...
| 文档 | 讲什么 | 何时看 |
|------|--------|--------|
| `plugins/project-evo/skills/project-evo/SKILL.md` | skill 本体概览 | 使用/修改 skill 前 |
| `plugins/project-evo/skills/project-evo/references/README.md` | 参考知识体系渐进索引（快速路由到场景到全量） | 找参考文档时 |
| `plugins/project-evo/skills/project-evo/verification/command-test-cases.md` | 规范检查命令 | 验证某项目是否符合骨架 |
...
1. **单一权威源**：skill 内容只在 `plugins/project-evo/skills/project-evo/` 维护；引用其项目的文档体系规则以该 skill 为准，本文件不重复。[经验： 双份漂移踩坑]
