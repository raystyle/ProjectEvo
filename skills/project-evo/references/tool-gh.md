...
`gh search code --json` 返回的 `items[].sha` = **blob sha**(→文件内容);`html_url` 中的 sha = **commit sha**(→变更历史)。这是 gh 与 git 互查的咬合点:gh 定位拿双 sha → git clone 后 `git show <commit_sha>:<path>` 对账。[经验: 同上速查表一级结论]
...
| bool flag 空格传值 | `--archived true` → `[]` | `--archived=true` |
| `--filename "*.glob"` 通配 | `[]` | 用具体文件名(`--filename sched.h`) |
| 内联多 qualifier 互斥 | `language:go repo:x` → `[]` | 独立 flag `--language` + `--repo` |
| `--json` 字段硬校验 | `--json nope` → exit 1 | 用 `--help` 列出的字段 |
...
- 带 `--jq` 直接出字段,省 jq 管道
- 限流时回退策略:匿名 curl `api.github.com` → gh api(认证)→ 重试退避 [经验: 同上]
