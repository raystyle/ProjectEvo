...
改已有或新建 `.docx` / `.xlsx` / `.pptx`，且要用可脚本化、可 `--json` 的命令而不是 GUI。客户端自带的 docx/pptx 技能仍可用；本 skill 管 OfficeCLI 通道与本仓实证过的纪律。斜杠命令 `/project-evo:office-cli [which|smoke]`。
...
定位二进制：`uv run plugins/project-evo/skills/office-pro/scripts/which.py`
