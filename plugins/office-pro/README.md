...
OfficeCLI 专业面插件。核心 skill 为 `office-pro`（显示 `office-pro:office-pro`）。
...
Codex：在 `/plugins` 里装 `office-pro`。二进制另按 `skills/office-pro/references/install.md` 钉资产，插件不代装。

## 用法

技能按意图路由自动触发（改 pptx/docx/xlsx、OfficeCLI、幻灯片偏位、稿面事实核查）。入口 `skills/office-pro/SKILL.md`。

斜杠命令（Claude Code 面）：

- `/office-pro:office [which|smoke]` 定位二进制或三类文件冒烟

```powershell
uv run plugins/office-pro/skills/office-pro/scripts/which.py
uv run plugins/office-pro/skills/office-pro/scripts/smoke.py
```
