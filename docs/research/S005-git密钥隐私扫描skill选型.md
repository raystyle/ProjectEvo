...
本仓 `evo` 的 `scan.py` 已扫工作区 + 本地 `git log -p --all`，并混扫 markdown 禁字。用户要的是**独立扫描 skill**：密钥/密码/隐私，脚本走 uv，对象含本地仓与 GitHub 历史提交。
...
| 本仓 evo scan | `skills/evo/scripts/scan.py` | 工作区+本地历史+md 禁字 | PEP 723 零依赖 | 无 GitHub 面；规则少；与文档禁字绑死 |
...
插件 `secret-scan`，skill 名 `secrets`。evo 的 `scan` 斜杠命令保留（文档禁字 + 浅密钥）。本 skill 专责密钥/隐私挖掘。
